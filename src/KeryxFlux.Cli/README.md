...
keryxflux validate patient-sync.yaml

# Validate directory
keryxflux validate ./dockets

# Verbose output
keryxflux validate patient-sync.yaml -v
```
...
keryxflux preview patient-sync.yaml --at "2025-01-15T15:00:00Z"

# Preview specific tenant only
keryxflux preview patient-sync.yaml --tenant HOSP001
```

**Output:**
```
Preview for 'patient-sync-poller' at 2025-01-15 15:00:00 UTC
...
? Endpoints:                                                   ?
?   Patient:                                                   ?
?     https://fhir.epic.com/MAIN_HOSPITAL/Patient?_lastUpdated=gt2025-01-15T14:00:00Z?
????????????????????????????????????????????????????????????????
```
...
keryxflux debug patient-sync.yaml --resolve

# Show multi-tenant expansion matrix
keryxflux debug patient-sync.yaml --expansion

# Debug at specific time
keryxflux debug patient-sync.yaml --at "2025-01-15T15:00:00Z"
```
